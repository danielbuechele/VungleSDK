...
//
+ (NSString*) value {
    return [VGOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [VGOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [VGOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [VGOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
